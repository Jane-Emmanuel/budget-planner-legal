# Simple Budget Planner – Privacy & Terms

This repository contains the official **Privacy Policy** and **Terms & Conditions** for the **Simple Budget Planner** app.

These documents are hosted publicly to satisfy **Google Play Store** requirements for privacy, legal compliance, and transparency regarding data use and affiliate links.

## Files Included

- **privacy-policy.html** – Explains what information the app collects (none), how it handles data, and affiliate disclosures.
- **terms.html** – Contains the app's terms of use, disclaimers, and affiliate disclosures.

## Purpose

These files provide:

- Transparency for users
- Legal protection for the developer
- Compliance with Google Play Store policies for apps containing affiliate links or in-app promotional content.

https://Jane-Emmanuel.github.io/budget-planner-legal/privacy-policy.html

https://Jane-Emmanuel.github.io/budget-planner-legal/terms.html

## Contact

For questions regarding these policies, contact:

**Email:** thetechthriller@gmail.com

